...
class Rest extends Dbc{
	...
	public function getUser($rfid_id) {
		$sqlQuery = '';
		if(!empty($rfid_id)){
		...
		while($empRec = mysqli_fetch_assoc($resultDat)) {
			if (password_verify($rfid_id, $empRec['rfid'])) {
			$userinfo[] = $empRec;
			}
		}
		return $userinfo;
